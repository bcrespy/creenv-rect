...
class Color {
  /**
   * Instanciate a new color using the color components sent as parameter. 
   * Because we sometimes want the color operations to be as fast as possible,
   * this class aims at performance. This constructor does not test if a string
   * gets passed as argument, or an array... etc. If you want to create a color
   * from a string look at Color.fromString static method 
   * 
   * @param {number} red the red component value [0;255]
   * @param {number} green the greend component value [0;255]
   * @param {number} blue the blue component value [0;255]
   * @param {number} alpha the alpha component value [0;1]
   */
  constructor (red = 0, green = 0, blue = 0, alpha = 1) {  
     ...
     * @private
     */
    this.components = [ red, green, blue, alpha ];
  }
  ...
  static fromString (colorString) {
    ...
    let color = colorstring.get(colorString);
    if (color === null) {
  ...
  /**
   * Instanciate a new color, copy of colro in argument 
   * 
   * @param {Color} color the color to copy
   * 
   * @return {Color} a new color, copy of color
   */
  static fromColor (color) {
    return new Color(...color.components);
  }

  /**
   * @param {Array.<number>} components a 3/4 components array
   * 
   * @return {Color} a new color which components are those sent in arguments
   */
