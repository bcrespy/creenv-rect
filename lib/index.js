...
class Colorpicker {
  ...
  generateDOM () {
    ...
    this.pickingBoxDom.classList.add(classname("picking-box"), ...classesFromDirection(this.direction).split(' '), "hidden");
  ...
  /**
   * Opens the picking box
   * Does not perform a check to see if weither the pincking box should
   * be opened or not 
   */
  openPickingBox () {
  ...
  /**
   * generates an input given the channel name - red, green, blue, alpha -.
   * 
   * @param {string} component a string of the channel to generate
   */
